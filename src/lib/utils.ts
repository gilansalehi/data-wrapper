export const noop = () => { };
export const idop = (a: unknown) => a;
export const q = (s: string, ctx: Element | Document = document) => [...ctx.querySelectorAll(s)] as Element[];
export const qcb = (s: string, cb: (el: Element) => unknown = idop, ctx: Element | Document = document) => q(s, ctx).map(cb);
